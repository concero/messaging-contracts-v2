...
// clf/src/messageReport/constants/bitOffsets.ts
var MESSAGE_REPORT_RESULT_SIZES = {
  VERSION: 1,
  REPORT_TYPE: 1,
  OPERATOR_ADDRESS: 32,
...
function packResult(result) {
  ...
  let offset = 0;
  const res = new Uint8Array(MESSAGE_REPORT_RESULT_SIZES.DST_CHAIN_DATA_LENGTH + dstChainDataLength + MESSAGE_REPORT_RESULT_SIZES.OPERATOR_LENGTH + operatorLength + MESSAGE_REPORT_RESULT_SIZES.OPERATORS_COUNT + operatorsLength * MESSAGE_REPORT_RESULT_SIZES.OPERATOR_BYTES);
  res[offset++] = result.version;
  res[offset++] = result.reportType;
  res.set(new Uint8Array([operatorLength]), offset);
  offset += MESSAGE_REPORT_RESULT_SIZES.OPERATOR_LENGTH;
  ...
  offset += MESSAGE_REPORT_RESULT_SIZES.DST_CHAIN_DATA_LENGTH;
  ...
  for (const operator of result.allowedOperators) {
    const operatorBytes2 = new Uint8Array(Buffer.from(operator.replace(/^0x/, ""), "hex"));
    res.set(operatorBytes2, offset);
    offset += operatorBytes2.length;
  }
  return res;
}

// clf/src/common/healthy-rpcs.json
...
// clf/src/messageReport/constants/viemChains.ts
var viemChains = {
  1: mainnet,
  8453: base
};
...
function decodeSrcChainData(srcChainSelector, srcChainData) {
  try {
    ...
    console.log("Error decoding srcChainData:", error);
...
async function main(bytesArgs) {
  try {
    ...
    const decodedInternalMessageConfig = args.internalMessageConfig;
...
// clf/src/messageReport/test.ts
async function test() {
  const messageConfig = 1n << BigInt(INTERNAL_MESSAGE_CONFIG_OFFSETS.VERSION) | 1n << BigInt(INTERNAL_MESSAGE_CONFIG_OFFSETS.SRC_CHAIN) | 8453n << BigInt(INTERNAL_MESSAGE_CONFIG_OFFSETS.DST_CHAIN) | 1n << BigInt(INTERNAL_MESSAGE_CONFIG_OFFSETS.MIN_SRC_CONF) | 1n << BigInt(INTERNAL_MESSAGE_CONFIG_OFFSETS.MIN_DST_CONF) | 0n << BigInt(INTERNAL_MESSAGE_CONFIG_OFFSETS.RELAYER) | 0n << BigInt(INTERNAL_MESSAGE_CONFIG_OFFSETS.CALLBACKABLE);
