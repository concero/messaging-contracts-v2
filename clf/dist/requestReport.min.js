...
// clf/src/requestReport/packResult.ts
function packResult(_messageId, _messageHash, chainSelector, blockNumber) {
  const encodeUint64 = (num2) => {
    const hexStr = num2.toString(16).padStart(16, "0");
...
// clf/src/requestReport/constants.ts
...
// clf/src/requestReport/healthy-rpcs.json
...
// clf/src/requestReport/viemClient.ts
...
// clf/src/requestReport/messageConfig.ts
...
function validateInternalMessageConfig(config) {
  console.log(config);
  if (config.version === 0)
    throw new Error(16 /* CONFIG_INVALID_VERSION */);
  if (config.relayerConfig > 255)
    throw new Error(17 /* CONFIG_INVALID_RELAYER_CONFIG */);
  if (config.minSrcConfirmations === 0)
    throw new Error(18 /* CONFIG_INVALID_MIN_SRC_CONFIRMATIONS */);
  if (config.minDstConfirmations === 0)
    throw new Error(19 /* CONFIG_INVALID_MIN_DST_CONFIRMATIONS */);
  if (!viemChains[config.srcChainSelector])
    throw new Error(20 /* CONFIG_INVALID_SRC_CHAIN_SELECTOR */);
  if (!viemChains[config.dstChainSelector])
    throw new Error(21 /* CONFIG_INVALID_DST_CHAIN_SELECTOR */);
}

// clf/src/requestReport/utils.ts
function getCohortId(operator, cohortsCount) {
  return parseInt(operator.slice(2), 16) % cohortsCount;
}
function pick(array, n) {
  if (n > array.length) {
    throw new Error(22 /* INVALID_OPERATOR_COUNT */);
  }
...
async function fetchConceroMessage(client, routerAddress, messageId, blockNumber) {
  ...
  if (!logs.length)
    throw new Error(3 /* EVENT_NOT_FOUND */);
  return logs[0];
}

// clf/src/requestReport/getAllowedOperators.ts
async function getAllowedOperators(client, chainType, messageId) {
  try {
    ...
    if (cohortsCount <= 0) {
      throw new Error(26 /* INVALID_COHORTS_COUNT */);
    }
    const messageCohort = getCohortId(messageId, cohortsCount);
    ...
    if (!registeredOperators.length) {
      throw new Error(23 /* NO_REGISTERED_OPERATORS */);
    }
    const allowedOperators = registeredOperators.filter((operator) => getCohortId(operator, cohortsCount) === messageCohort);
    if (!allowedOperators.length) {
      throw new Error(24 /* NO_ALLOWED_OPERATORS */);
    }
    return allowedOperators;
  } catch (error) {
    console.error("Error in getAllowedOperators:", error);
    throw error instanceof Error ? error : new Error(25 /* OPERATOR_SELECTION_FAILED */);
  }
}
...
async function main(bytesArgs) {
  if (bytesArgs.length < 6)
    throw new Error(15 /* INVALID_INPUT */);
  ...
  if (recomputedMessageHashSum !== messageHashSum)
    throw new Error(4 /* INVALID_HASHSUM */);
  const operators = await getAllowedOperators(client, 0 /* EVM */, messageId);
