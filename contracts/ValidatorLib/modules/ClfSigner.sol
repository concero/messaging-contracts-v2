...
abstract contract ClfSigner {
    ...
    address internal immutable i_conceroValidator;
    uint64 internal immutable i_conceroValidatorSubId;
    ...
    constructor(
        ...
        i_conceroValidatorSubId = conceroValidatorSubId;
    }
    ...
    function _isValidClfReportOnChainMetadata(
        Types.ClfReportOnchainMetadata memory onchainMetadata
    ) internal view returns (bool) {
        if (onchainMetadata.client != i_conceroValidator) {
            return false;
        }
        if (onchainMetadata.subscriptionId != i_conceroValidatorSubId) {
            return false;
        }
        return true;
    }
     ...
     */
    function _isValidClfReportSignatures(
        Types.ClfDonReportSubmission memory reportSubmission
    ) internal view returns (bool) {
        ...
        if (rs.length != ss.length || rs.length != expectedNumSignatures) {
            return false; // Incorrect number of signatures
        }
        ...
        for (uint256 i; i < rs.length; ++i) {
            ...
            if (!_isAuthorizedClfSigner(signer)) {
                return false;
            }
