...
class BaseError extends Error {
  constructor(shortMessage, args = {}) {
    ...
    super(message6, args.cause ? { cause: args.cause } : undefined);
...
var anumber, isBytes, abytes, aexists, aoutput;
var init__assert = __esm(() => {
  ...
  abytes = function(b, ...lengths) {
    if (!isBytes(b))
      throw new Error("Uint8Array expected");
    if (lengths.length > 0 && !lengths.includes(b.length))
      throw new Error("Uint8Array expected of length " + lengths + ", got length=" + b.length);
  };
  aexists = function(instance, checkFinished = true) {
    if (instance.destroyed)
      throw new Error("Hash instance has been destroyed");
    if (checkFinished && instance.finished)
      throw new Error("Hash#digest() has already been called");
  };
  aoutput = function(out, instance) {
    abytes(out);
...
var init_slice = __esm(() => {
  ...
  init_isHex();
  init_size();
  assertStartOffset = function(value2, start) {
  ...
  assertEndOffset = function(value2, start, end) {
...
var init_encodeAbiParameters = __esm(() => {
  ...
  prepareParam = function({ param, value: value2 }) {
  ...
  encodeArray = function(value2, { length, param }) {
    ...
    const preparedParams = [];
    for (let i = 0;i < value2.length; i++) {
      const preparedParam = prepareParam({ param, value: value2[i] });
  ...
  encodeBytes = function(value2, { param }) {
  ...
  encodeNumber = function(value2, { signed, size: size6 = 256 }) {
  ...
  encodeTuple = function(value2, { param }) {
    let dynamic = false;
    const preparedParams = [];
    for (let i = 0;i < param.components.length; i++) {
      const param_ = param.components[i];
      const index = Array.isArray(value2) ? i : param_.name;
...
function prepareEncodeFunctionData(parameters) {
  const { abi: abi5, args, functionName } = parameters;
  let abiItem = abi5[0];
  if (functionName) {
...
function encodeFunctionData(parameters) {
  ...
  const { abi: abi5, functionName } = (() => {
    if (parameters.abi.length === 1 && parameters.functionName?.startsWith("0x"))
      return parameters;
    return prepareEncodeFunctionData(parameters);
  })();
  const abiItem = abi5[0];
...
function decodeAbiParameters(params, data3) {
  const bytes = typeof data3 === "string" ? hexToBytes2(data3) : data3;
  const cursor3 = createCursor(bytes);
...
var init_decodeAbiParameters = __esm(() => {
  ...
  decodeAddress = function(cursor3) {
    const value2 = cursor3.readBytes(32);
  ...
  decodeBytes = function(cursor3, param, { staticPosition }) {
    ...
    const value2 = bytesToHex2(cursor3.readBytes(Number.parseInt(size7), 32));
    return [value2, 32];
  };
  decodeNumber = function(cursor3, param) {
    ...
    const value2 = cursor3.readBytes(32);
    return [
      size7 > 48 ? bytesToBigInt(value2, { signed }) : bytesToNumber(value2, { signed }),
      32
    ];
  };
  decodeTuple = function(cursor3, param, { staticPosition }) {
    ...
    if (hasDynamicChild(param)) {
      ...
      for (let i = 0;i < param.components.length; ++i) {
        ...
        consumed += consumed_;
        value2[hasUnnamedChild ? i : component?.name] = data3;
      }
      cursor3.setPosition(staticPosition + 32);
      return [value2, 32];
    }
  ...
  decodeString = function(cursor3, { staticPosition }) {
    ...
    const value2 = bytesToString(trim(data3));
    cursor3.setPosition(staticPosition + 32);
    return [value2, 32];
  };
...
function decodeFunctionResult(parameters) {
  const { abi: abi7, args, functionName, data: data3 } = parameters;
  let abiItem = abi7[0];
  if (functionName) {
    const item = getAbiItem({ abi: abi7, args, name: functionName });
    if (!item)
...
var assertInt64String = function(value) {
  if (!/^-?[0-9]+$/.test(value)) {
    throw new Error("invalid int64: " + value);
  }
};
var assertUInt64String = function(value) {
  if (!/^[0-9]+$/.test(value)) {
    throw new Error("invalid uint64: " + value);
  }
};
...
var isWrapperTypeName = function(name) {
...
var initMessage = function(messageDesc, message, init) {
...
var initScalar = function(field, value) {
  if (field.scalar == ScalarType.BYTES) {
    return toU8Arr(value);
  }
  return value;
};
var initMap = function(field, value) {
  if (isObject(value)) {
    if (field.scalar == ScalarType.BYTES) {
...
var initList = function(field, value) {
...
var toMessage = function(field, value) {
...
var toU8Arr = function(value) {
  return Array.isArray(value) ? new Uint8Array(value) : value;
};
var convertObjectValues = function(obj, fn) {
  const ret = {};
...
var createZeroMessage = function(desc) {
  let msg;
...
var needsPrototypeChain = function(desc) {
...
var createZeroField = function(field) {
  if (field.kind == "oneof") {
...
var assertFloat32 = function(arg) {
  if (typeof arg == "string") {
    const o = arg;
    arg = Number(arg);
...
var checkSingular = function(field, value) {
...
var checkScalarValue = function(value, scalar3) {
  switch (scalar3) {
    case ScalarType.DOUBLE:
...
var reasonSingular = function(field, val, details) {
...
var formatReflectMessage = function(desc) {
  return `ReflectMessage (${desc.typeName})`;
};
var formatReflectList = function(field) {
...
var formatReflectMap = function(field) {
...
var scalarTypeDescription = function(scalar3) {
  switch (scalar3) {
    case ScalarType.STRING:
      return "string";
    case ScalarType.BOOL:
...
var assertOwn = function(owner, member) {
  if (member.parent.typeName !== owner.$typeName) {
    throw new FieldError(member, `cannot use ${member.toString()} with message ${owner.$typeName}`, "ForeignFieldError");
  }
};
var messageToLocal = function(field, value) {
...
var messageToReflect = function(field, value, check) {
  if (value !== undefined) {
...
var listItemToLocal = function(field, value) {
  if (field.listKind == "message") {
    return messageToLocal(field, value);
  }
  return longToLocal(field, value);
};
var listItemToReflect = function(field, value, check) {
  if (field.listKind == "message") {
    return messageToReflect(field, value, check);
  }
  return longToReflect(field, value);
};
var mapValueToLocal = function(field, value) {
  if (field.mapKind == "message") {
    return messageToLocal(field, value);
  }
  return longToLocal(field, value);
};
var mapValueToReflect = function(field, value, check) {
  if (field.mapKind == "message") {
    return messageToReflect(field, value, check);
  }
  return value;
};
var mapKeyToLocal = function(key) {
  return typeof key == "string" || typeof key == "number" ? key : String(key);
};
var mapKeyToReflect = function(key, type) {
  switch (type) {
    case ScalarType.STRING:
      return key;
    case ScalarType.INT32:
...
var longToReflect = function(field, value) {
  switch (field.scalar) {
    case ScalarType.INT64:
    case ScalarType.SFIXED64:
    case ScalarType.SINT64:
...
var longToLocal = function(field, value) {
  switch (field.scalar) {
    case ScalarType.INT64:
    case ScalarType.SFIXED64:
    case ScalarType.SINT64:
...
var wktStructToReflect = function(json) {
  const struct = {
...
var wktStructToLocal = function(val) {
  const json = {};
...
var wktValueToLocal = function(val) {
...
var wktValueToReflect = function(json) {
  const value = {
...
var getEncodeTable = function(encoding) {
  if (!encodeTableStd) {
...
var unescapeBytesDefaultValue = function(str) {
  const b = [];
  const input = {
    tail: str,
    c: "",
...
var createBaseRegistry = function() {
  const types = new Map;
...
var addFile = function(proto, reg) {
  var _a, _b;
  const file = {
    kind: "file",
    proto,
...
var addExtensions = function(desc, reg) {
  switch (desc.kind) {
    case "file":
...
var addFields = function(message, reg, mapEntries) {
...
var addEnum = function(proto, file, parent, reg) {
  var _a, _b, _c, _d, _e;
...
var addMessage = function(proto, file, parent, reg, mapEntries) {
  var _a, _b, _c, _d;
  const desc = {
    kind: "message",
    proto,
...
var addService = function(proto, file, reg) {
  var _a, _b;
  const desc = {
    kind: "service",
    proto,
...
var newMethod = function(proto, parent, reg) {
  var _a, _b, _c, _d;
  let methodKind;
...
var newOneof = function(proto, parent) {
  return {
    kind: "oneof",
    proto,
    deprecated: false,
...
var newField = function(proto, parentOrFile, reg, oneof, mapEntries) {
  var _a, _b, _c;
...
var getFileEdition = function(proto) {
  switch (proto.syntax) {
    case "":
    case "proto2":
...
var findFileDependencies = function(proto, reg) {
...
var findEnumSharedPrefix = function(enumName, values) {
...
var camelToSnakeCase = function(camel) {
  return (camel.substring(0, 1) + camel.substring(1).replace(/[A-Z]/g, (c) => "_" + c)).toLowerCase();
};
var makeTypeName = function(proto, parent, file) {
  let typeName;
  if (parent) {
...
var trimLeadingDot = function(typeName) {
  return typeName.startsWith(".") ? typeName.substring(1) : typeName;
};
var findOneof = function(proto, allOneofs) {
...
var getFieldPresence = function(proto, oneof, isExtension, parent) {
...
var isPackedField = function(proto, parent) {
  if (proto.label != LABEL_REPEATED) {
    return false;
  }
  switch (proto.type) {
...
var findMapEntryFields = function(mapEntry) {
  const key = mapEntry.fields.find((f) => f.number === 1);
  const value = mapEntry.fields.find((f) => f.number === 2);
  assert(key && key.fieldKind == "scalar" && key.scalar != ScalarType.BYTES && key.scalar != ScalarType.FLOAT && key.scalar != ScalarType.DOUBLE && value && value.fieldKind != "list" && value.fieldKind != "map");
  return { key, value };
};
var isEnumOpen = function(desc) {
  var _a;
...
var isDelimitedEncoding = function(proto, parent) {
...
var resolveFeature = function(name, ref) {
  var _a, _b;
...
var bootDescriptorProto = function(init) {
...
var bootFieldDescriptorProto = function(init) {
  const proto = Object.create({
    label: 1,
    typeName: "",
    extendee: "",
...
var bootFieldOptions = function(init) {
  var _a, _b, _c;
  const proto = Object.create({
    ctype: 0,
    packed: false,
...
var bootEnumDescriptorProto = function(init) {
  const proto = Object.create({
    visibility: 0
  });
  return Object.assign(proto, {
...
var readMessage = function(message2, reader, options, delimited, lengthOrDelimitedFieldNo) {
  var _a;
  const end = delimited ? reader.len : reader.pos + lengthOrDelimitedFieldNo;
  let fieldNo;
  let wireType;
  const unknownFields = (_a = message2.getUnknown()) !== null && _a !== undefined ? _a : [];
...
var readMapEntry = function(reader, map, options) {
...
var readListField = function(reader, wireType, list, options) {
  var _a;
...
var readMessageField = function(reader, options, field, mergeMessage) {
  const delimited = field.delimitedEncoding;
  const message2 = mergeMessage !== null && mergeMessage !== undefined ? mergeMessage : reflect(field.message, undefined, false);
  readMessage(message2, reader, options, delimited, delimited ? field.number : reader.uint32());
  return message2;
};
var readScalar = function(reader, type) {
  switch (type) {
    case ScalarType.STRING:
...
function toBinary(schema, message4, options) {
  return writeFields(new BinaryWriter, makeWriteOptions(options), reflect(schema, message4)).finish();
}
var writeFields = function(writer, opts, msg) {
  var _a;
...
var writeScalar = function(writer, msgName, fieldName, scalarType, fieldNo, value) {
  writeScalarValue(writer.tag(fieldNo, writeTypeOfScalar(scalarType)), msgName, fieldName, scalarType, value);
};
var writeMessageField = function(writer, opts, field, message4) {
  if (field.delimitedEncoding) {
    writeFields(writer.tag(field.number, WireType.StartGroup), opts, message4).tag(field.number, WireType.EndGroup);
  } else {
    writeFields(writer.tag(field.number, WireType.LengthDelimited).fork(), opts, message4).join();
  }
};
var writeListField = function(writer, opts, field, list) {
  var _a;
  if (field.listKind == "message") {
...
var writeMapEntry = function(writer, opts, field, key, value) {
  var _a;
  writer.tag(field.number, WireType.LengthDelimited).fork();
...
var writeScalarValue = function(writer, msgName, fieldName, type, value) {
  try {
    switch (type) {
      case ScalarType.STRING:
        writer.string(value);
...
var writeTypeOfScalar = function(type) {
  switch (type) {
    case ScalarType.BYTES:
    case ScalarType.STRING:
...
function anyPack(schema, message4, into) {
  let ret = false;
  if (!into) {
    into = create(AnySchema);
    ret = true;
  }
  into.value = toBinary(schema, message4);
  into.typeUrl = typeNameToUrl(message4.$typeName);
...
var typeNameToUrl = function(name) {
  return `type.googleapis.com/${name}`;
};
var typeUrlToName = function(url) {
...
function setExtension(message6, extension, value) {
  var _a;
  assertExtendee(extension, message6);
  const ufs = ((_a = message6.$unknown) !== null && _a !== undefined ? _a : []).filter((uf) => uf.no !== extension.number);
...
var assertExtendee = function(extension, message6) {
  if (extension.extendee.typeName != message6.$typeName) {
...
var readMessage2 = function(msg, json, opts) {
  var _a;
...
var readField2 = function(msg, field, json, opts) {
  switch (field.fieldKind) {
    case "scalar":
...
var readMapField = function(map, json, opts) {
  if (json === null) {
    return;
  }
  const field = map.field();
...
var readListField2 = function(list, json, opts) {
  if (json === null) {
    return;
  }
  const field = list.field();
...
var readMessageField2 = function(msg, field, json, opts) {
...
var readEnumField = function(msg, field, json, opts) {
...
var readScalarField = function(msg, field, json) {
...
var readEnum = function(desc, json, ignoreUnknownFields, nullAsZeroValue) {
  if (json === null) {
...
var scalarFromJson = function(field, json, nullAsZeroValue) {
  if (json === null) {
    if (nullAsZeroValue) {
    ...
    case ScalarType.BYTES:
      if (typeof json == "string") {
        ...
        try {
          ...
          const message6 = e instanceof Error ? e.message : String(e);
          throw new FieldError(field, message6);
        }
      }
      break;
  }
...
var mapKeyFromJson = function(type, json) {
  switch (type) {
    case ScalarType.BOOL:
      switch (json) {
        case "true":
...
var int32FromJson = function(json) {
  if (typeof json == "string") {
    if (json === "") {
      return json;
    }
...
var tryWktFromJson = function(msg, jsonValue, opts) {
...
var anyFromJson = function(any, json, opts) {
  var _a;
  if (json === null || Array.isArray(json) || typeof json != "object") {
...
var timestampFromJson = function(timestamp, json) {
  if (typeof json !== "string") {
...
var durationFromJson = function(duration, json) {
  if (typeof json !== "string") {
...
var fieldMaskFromJson = function(fieldMask, json) {
  if (typeof json !== "string") {
...
var structFromJson = function(struct, json) {
...
var valueFromJson = function(value, json) {
  switch (typeof json) {
    case "number":
      value.kind = { case: "numberValue", value: json };
      break;
...
var listValueFromJson = function(listValue, json) {
  if (!Array.isArray(json)) {
...
var ValueSchema2 = messageDesc(file_values_v1_values, 0);
var BigIntSchema = messageDesc(file_values_v1_values, 1);
var MapSchema = messageDesc(file_values_v1_values, 2);
var ListSchema = messageDesc(file_values_v1_values, 3);
var DecimalSchema = messageDesc(file_values_v1_values, 4);

// ../../../node_modules/@chainlink/cre-sdk/dist/generated/sdk/v1alpha/sdk_pb.js
var file_sdk_v1alpha_sdk = fileDesc("", [file_google_protobuf_any, file_google_protobuf_empty, file_values_v1_values]);
var SimpleConsensusInputsSchema = messageDesc(file_sdk_v1alpha_sdk, 0);
var FieldsMapSchema = messageDesc(file_sdk_v1alpha_sdk, 1);
var ConsensusDescriptorSchema = messageDesc(file_sdk_v1alpha_sdk, 2);
var ReportRequestSchema = messageDesc(file_sdk_v1alpha_sdk, 3);
var ReportResponseSchema = messageDesc(file_sdk_v1alpha_sdk, 4);
var CapabilityRequestSchema = messageDesc(file_sdk_v1alpha_sdk, 6);
var TriggerSubscriptionRequestSchema = messageDesc(file_sdk_v1alpha_sdk, 9);
var AwaitCapabilitiesRequestSchema = messageDesc(file_sdk_v1alpha_sdk, 11);
var AwaitCapabilitiesResponseSchema = messageDesc(file_sdk_v1alpha_sdk, 12);
var ExecuteRequestSchema = messageDesc(file_sdk_v1alpha_sdk, 13);
var ExecutionResultSchema = messageDesc(file_sdk_v1alpha_sdk, 14);
var GetSecretsRequestSchema = messageDesc(file_sdk_v1alpha_sdk, 15);
var AwaitSecretsRequestSchema = messageDesc(file_sdk_v1alpha_sdk, 16);
var AwaitSecretsResponseSchema = messageDesc(file_sdk_v1alpha_sdk, 17);
var SecretRequestSchema = messageDesc(file_sdk_v1alpha_sdk, 18);
var AggregationType;
...
var file_tools_generator_v1alpha_cre_metadata = fileDesc("", [file_google_protobuf_descriptor, file_sdk_v1alpha_sdk]);

// ../../../node_modules/@chainlink/cre-sdk/dist/generated/capabilities/blockchain/evm/v1alpha/client_pb.js
var file_capabilities_blockchain_evm_v1alpha_client = fileDesc("", [
  file_google_protobuf_empty,
  file_sdk_v1alpha_sdk,
  file_tools_generator_v1alpha_cre_metadata,
  file_values_v1_values
]);
var FilterLogTriggerRequestSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 1);
var CallContractRequestSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 2);
var CallContractReplySchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 3);
var FilterLogsRequestSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 4);
var FilterLogsReplySchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 5);
var LogSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 6);
var BalanceAtRequestSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 10);
var BalanceAtReplySchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 11);
var EstimateGasRequestSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 12);
var EstimateGasReplySchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 13);
var GetTransactionByHashRequestSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 14);
var GetTransactionByHashReplySchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 15);
var GetTransactionReceiptRequestSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 17);
var GetTransactionReceiptReplySchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 18);
var HeaderByNumberRequestSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 20);
var HeaderByNumberReplySchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 21);
var RegisterLogTrackingRequestSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 23);
var UnregisterLogTrackingRequestSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 25);
var WriteReportRequestSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 26);
var GasConfigSchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 27);
var WriteReportReplySchema = messageDesc(file_capabilities_blockchain_evm_v1alpha_client, 28);
var ConfidenceLevel;
...
var RequestSchema = messageDesc(file_capabilities_networking_http_v1alpha_client, 1);
var ResponseSchema = messageDesc(file_capabilities_networking_http_v1alpha_client, 2);
...
var file_capabilities_scheduler_cron_v1_trigger = fileDesc("", [file_google_protobuf_timestamp, file_tools_generator_v1alpha_cre_metadata]);
var ConfigSchema2 = messageDesc(file_capabilities_scheduler_cron_v1_trigger, 0);
var PayloadSchema2 = messageDesc(file_capabilities_scheduler_cron_v1_trigger, 1);
...
var sendReport = function(runtime2, report5, fn) {
  const rawReport = report5.x_generatedCodeOnly_unwrap();
  const request = fn(rawReport);
  return this.sendRequest(runtime2, request);
};
var sendRequesterSendReport = function(report5, fn) {
  const rawReport = report5.x_generatedCodeOnly_unwrap();
  const request = fn(rawReport);
...
// ../../../node_modules/@chainlink/cre-sdk/dist/sdk/utils/chain-selectors/getNetwork.js
...
var simpleDescriptor = function(agg) {
...
var isValueProto = function(value) {
  ...
  switch (issue.code) {
    case ZodIssueCode.invalid_type:
      if (issue.received === ZodParsedType.undefined) {
        ...
        message6 = `Expected ${issue.expected}, received ${issue.received}`;
      }
      break;
    case ZodIssueCode.invalid_literal:
      message6 = `Invalid literal value, expected ${JSON.stringify(issue.expected, util.jsonStringifyReplacer)}`;
      break;
    case ZodIssueCode.unrecognized_keys:
      message6 = `Unrecognized key(s) in object: ${util.joinValues(issue.keys, ", ")}`;
      break;
    case ZodIssueCode.invalid_union:
      message6 = `Invalid input`;
      break;
    case ZodIssueCode.invalid_union_discriminator:
      message6 = `Invalid discriminator value. Expected ${util.joinValues(issue.options)}`;
      break;
    case ZodIssueCode.invalid_enum_value:
      message6 = `Invalid enum value. Expected ${util.joinValues(issue.options)}, received '${issue.received}'`;
      break;
    case ZodIssueCode.invalid_arguments:
      message6 = `Invalid function arguments`;
      break;
    case ZodIssueCode.invalid_return_type:
      message6 = `Invalid function return type`;
      break;
    case ZodIssueCode.invalid_date:
      message6 = `Invalid date`;
      break;
    case ZodIssueCode.invalid_string:
      if (typeof issue.validation === "object") {
        if ("includes" in issue.validation) {
          message6 = `Invalid input: must include "${issue.validation.includes}"`;
          if (typeof issue.validation.position === "number") {
            message6 = `${message6} at one or more positions greater than or equal to ${issue.validation.position}`;
          }
        } else if ("startsWith" in issue.validation) {
          message6 = `Invalid input: must start with "${issue.validation.startsWith}"`;
        } else if ("endsWith" in issue.validation) {
          message6 = `Invalid input: must end with "${issue.validation.endsWith}"`;
        } else {
          util.assertNever(issue.validation);
        }
      } else if (issue.validation !== "regex") {
        message6 = `Invalid ${issue.validation}`;
      } else {
        message6 = "Invalid";
      }
      break;
    case ZodIssueCode.too_small:
      if (issue.type === "array")
        message6 = `Array must contain ${issue.exact ? "exactly" : issue.inclusive ? `at least` : `more than`} ${issue.minimum} element(s)`;
      else if (issue.type === "string")
        message6 = `String must contain ${issue.exact ? "exactly" : issue.inclusive ? `at least` : `over`} ${issue.minimum} character(s)`;
      else if (issue.type === "number")
        message6 = `Number must be ${issue.exact ? `exactly equal to ` : issue.inclusive ? `greater than or equal to ` : `greater than `}${issue.minimum}`;
      else if (issue.type === "bigint")
        message6 = `Number must be ${issue.exact ? `exactly equal to ` : issue.inclusive ? `greater than or equal to ` : `greater than `}${issue.minimum}`;
      else if (issue.type === "date")
        message6 = `Date must be ${issue.exact ? `exactly equal to ` : issue.inclusive ? `greater than or equal to ` : `greater than `}${new Date(Number(issue.minimum))}`;
      else
        message6 = "Invalid input";
      break;
    case ZodIssueCode.too_big:
      if (issue.type === "array")
        message6 = `Array must contain ${issue.exact ? `exactly` : issue.inclusive ? `at most` : `less than`} ${issue.maximum} element(s)`;
      else if (issue.type === "string")
        message6 = `String must contain ${issue.exact ? `exactly` : issue.inclusive ? `at most` : `under`} ${issue.maximum} character(s)`;
      else if (issue.type === "number")
        message6 = `Number must be ${issue.exact ? `exactly` : issue.inclusive ? `less than or equal to` : `less than`} ${issue.maximum}`;
      else if (issue.type === "bigint")
        message6 = `BigInt must be ${issue.exact ? `exactly` : issue.inclusive ? `less than or equal to` : `less than`} ${issue.maximum}`;
      else if (issue.type === "date")
        message6 = `Date must be ${issue.exact ? `exactly` : issue.inclusive ? `smaller than or equal to` : `smaller than`} ${new Date(Number(issue.maximum))}`;
      else
        message6 = "Invalid input";
      break;
    ...
    case ZodIssueCode.invalid_intersection_types:
      message6 = `Intersection results could not be merged`;
      break;
    case ZodIssueCode.not_multiple_of:
      message6 = `Number must be a multiple of ${issue.multipleOf}`;
      break;
    case ZodIssueCode.not_finite:
      message6 = "Number must be finite";
      break;
    default:
      message6 = _ctx.defaultError;
      util.assertNever(issue);
  }
  return { message: message6 };
};
...
var errorUtil;
(function(errorUtil2) {
  errorUtil2.errToObj = (message6) => typeof message6 === "string" ? { message: message6 } : message6 || {};
  errorUtil2.toString = (message6) => typeof message6 === "string" ? message6 : message6?.message;
})(errorUtil || (errorUtil = {}));
...
var processCreateParams = function(params) {
  ...
  const customMap = (iss, ctx) => {
    const { message: message6 } = params;
    if (iss.code === "invalid_enum_value") {
      return { message: message6 ?? ctx.defaultError };
    }
    if (typeof ctx.data === "undefined") {
      return { message: message6 ?? required_error ?? ctx.defaultError };
    }
    if (iss.code !== "invalid_type")
      return { message: ctx.defaultError };
    return { message: message6 ?? invalid_type_error ?? ctx.defaultError };
  };
...
class ZodType {
  ...
  refine(check, message6) {
    const getIssueProperties = (val) => {
      if (typeof message6 === "string" || typeof message6 === "undefined") {
        return { message: message6 };
      } else if (typeof message6 === "function") {
        return message6(val);
      } else {
        return message6;
      }
...
class ZodString extends ZodType {
  ...
  _regex(regex, validation, message6) {
    return this.refinement((data) => regex.test(data), {
      validation,
      code: ZodIssueCode.invalid_string,
      ...errorUtil.errToObj(message6)
    });
  }
  _addCheck(check) {
  ...
  email(message6) {
    return this._addCheck({ kind: "email", ...errorUtil.errToObj(message6) });
  }
  url(message6) {
    return this._addCheck({ kind: "url", ...errorUtil.errToObj(message6) });
  }
  emoji(message6) {
    return this._addCheck({ kind: "emoji", ...errorUtil.errToObj(message6) });
  }
  uuid(message6) {
    return this._addCheck({ kind: "uuid", ...errorUtil.errToObj(message6) });
  }
  nanoid(message6) {
    return this._addCheck({ kind: "nanoid", ...errorUtil.errToObj(message6) });
  }
  cuid(message6) {
    return this._addCheck({ kind: "cuid", ...errorUtil.errToObj(message6) });
  }
  cuid2(message6) {
    return this._addCheck({ kind: "cuid2", ...errorUtil.errToObj(message6) });
  }
  ulid(message6) {
    return this._addCheck({ kind: "ulid", ...errorUtil.errToObj(message6) });
  }
  base64(message6) {
    return this._addCheck({ kind: "base64", ...errorUtil.errToObj(message6) });
  }
  base64url(message6) {
  ...
  date(message6) {
    return this._addCheck({ kind: "date", message: message6 });
  }
  time(options) {
    if (typeof options === "string") {
      return this._addCheck({
  ...
  duration(message6) {
    return this._addCheck({ kind: "duration", ...errorUtil.errToObj(message6) });
  }
  regex(regex, message6) {
    return this._addCheck({
      kind: "regex",
      regex,
      ...errorUtil.errToObj(message6)
  ...
  nonempty(message6) {
    return this.min(1, errorUtil.errToObj(message6));
  }
  trim() {
    return new ZodString({
      ...this._def,
...
class ZodNumber extends ZodType {
  ...
  gte(value2, message6) {
    return this.setLimit("min", value2, true, errorUtil.toString(message6));
  }
  gt(value2, message6) {
    return this.setLimit("min", value2, false, errorUtil.toString(message6));
  }
  lte(value2, message6) {
    return this.setLimit("max", value2, true, errorUtil.toString(message6));
  }
  lt(value2, message6) {
    return this.setLimit("max", value2, false, errorUtil.toString(message6));
  }
  setLimit(kind, value2, inclusive, message6) {
    return new ZodNumber({
      ...
      checks: [
        ...this._def.checks,
        {
          ...
          value: value2,
          inclusive,
          message: errorUtil.toString(message6)
        }
      ]
    });
  }
...
class ZodArray extends ZodType {
  ...
  min(minLength, message6) {
    return new ZodArray({
      ...this._def,
      minLength: { value: minLength, message: errorUtil.toString(message6) }
    });
  }
  max(maxLength, message6) {
    return new ZodArray({
      ...this._def,
      maxLength: { value: maxLength, message: errorUtil.toString(message6) }
    });
  }
  length(len, message6) {
    return new ZodArray({
      ...this._def,
      exactLength: { value: len, message: errorUtil.toString(message6) }
    });
  }
  nonempty(message6) {
    return this.min(1, message6);
  }
}
      ...
      ...message6 !== undefined ? {
        errorMap: (issue, ctx) => {
          ...
          if (issue.code === "unrecognized_keys")
            return {
              message: errorUtil.errToObj(message6).message ?? defaultError
            };
          return {
...
class ZodSet extends ZodType {
  ...
  min(minSize, message6) {
    return new ZodSet({
      ...this._def,
      minSize: { value: minSize, message: errorUtil.toString(message6) }
    });
  }
  max(maxSize, message6) {
    return new ZodSet({
      ...this._def,
      maxSize: { value: maxSize, message: errorUtil.toString(message6) }
    });
  }
  size(size, message6) {
    return this.min(size, message6).max(size, message6);
  }
  nonempty(message6) {
    return this.min(1, message6);
  }
...
class SecretsError extends Error {
  sceretRequest;
  error;
  constructor(sceretRequest, error4) {
    super(`error fetching ${sceretRequest}: ${error4}`);
...
class Runner {
  ...
  static async newRunner(configHandlerParams) {
    ...
    const config2 = await configHandler(request, configHandlerParams);
    return new Runner(config2, request);
  }
  static getRequest() {
  ...
  async run(initFn) {
    const runtime3 = new Runtime(this.config, 0, this.request.maxResponseSize);
    var result;
    try {
      const workflow2 = await initFn(this.config, {
        getSecret: runtime3.getSecret.bind(runtime3)
      });
      switch (this.request.request.case) {
        case "subscribe":
          ...
          break;
        case "trigger":
          result = this.handleExecutionPhase(this.request, workflow2, runtime3);
          break;
      }
    } catch (e) {
  ...
  async handleExecutionPhase(req, workflow2, runtime3) {
    ...
    if (Number.isFinite(index) && index >= 0 && index < workflow2.length) {
      ...
      try {
        const result = await entry.fn(runtime3, adapted);
...
// ../contracts/abi/BalanceReader.ts
var BalanceReader = [
  {
...
// ../contracts/abi/MessageEmitter.ts
var MessageEmitter = [
  {
    anonymous: false,
    inputs: [
...
// ../contracts/abi/ReserveManager.ts
var ReserveManager = [
  {
    anonymous: false,
    inputs: [
...
var fetchReserveInfo = (sendRequester, config2) => {
  const response = sendRequester.sendRequest({ url: config2.url }).result();
...
var fetchNativeTokenBalance = (runtime4, evmConfig, tokenHolderAddress) => {
  ...
  const contractCall = evmClient.callContract(runtime4, {
    call: encodeCallMsg({
      from: zeroAddress,
...
var getTotalSupply = (runtime4) => {
  const evms = runtime4.config.evms;
  let totalSupply = 0n;
...
var updateReserves = (runtime4, totalSupply, totalReserveScaled) => {
  const evmConfig = runtime4.config.evms[0];
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: evmConfig.chainSelectorName,
    isTestnet: true
  ...
  const evmClient = new cre.capabilities.EVMClient(network248.chainSelector.selector);
  runtime4.log(`Updating reserves totalSupply ${totalSupply.toString()} totalReserveScaled ${totalReserveScaled.toString()}`);
  const callData = encodeFunctionData({
    abi: ReserveManager,
  ...
  const reportResponse = runtime4.report({
    encodedPayload: hexToBase64(callData),
    encoderName: "evm",
    signingAlgo: "ecdsa",
    hashingAlgo: "keccak256"
  }).result();
  const resp = evmClient.writeReport(runtime4, {
  ...
  runtime4.log(`Write report transaction succeeded at txHash: ${bytesToHex(txHash)}`);
  return txHash.toString();
};
var doPOR = (runtime4) => {
  runtime4.log(`fetching por url ${runtime4.config.url}`);
  const httpCapability = new cre.capabilities.HTTPClient;
  const reserveInfo = httpCapability.sendRequest(runtime4, fetchReserveInfo, ConsensusAggregationByFields({
    lastUpdated: median,
    totalReserve: median
  }))(runtime4.config).result();
  runtime4.log(`ReserveInfo ${safeJsonStringify(reserveInfo)}`);
  const totalSupply = getTotalSupply(runtime4);
  runtime4.log(`TotalSupply ${totalSupply.toString()}`);
  const totalReserveScaled = BigInt(reserveInfo.totalReserve * 1000000000000000000);
  runtime4.log(`TotalReserveScaled ${totalReserveScaled.toString()}`);
  const nativeTokenBalance = fetchNativeTokenBalance(runtime4, runtime4.config.evms[0], runtime4.config.evms[0].tokenAddress);
  runtime4.log(`NativeTokenBalance ${nativeTokenBalance.toString()}`);
  const secretAddress = runtime4.getSecret({ id: "SECRET_ADDRESS" }).result();
  const secretAddressBalance = fetchNativeTokenBalance(runtime4, runtime4.config.evms[0], secretAddress.value);
  runtime4.log(`SecretAddressBalance ${secretAddressBalance.toString()}`);
  updateReserves(runtime4, totalSupply, totalReserveScaled);
  return reserveInfo.totalReserve.toString();
};
var getLastMessage = (runtime4, evmConfig, emitter) => {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: evmConfig.chainSelectorName,
    isTestnet: true
  ...
  const message6 = decodeFunctionResult({
    abi: MessageEmitter,
    functionName: "getLastMessage",
    data: bytesToHex(contractCall.data)
  });
...
var onCronTrigger = (runtime4, payload) => {
  if (!payload.scheduledExecutionTime) {
    throw new Error("Scheduled execution time is required");
  }
  runtime4.log("Running CronTrigger");
  return doPOR(runtime4);
};
var onLogTrigger = (runtime4, payload) => {
  runtime4.log("Running LogTrigger");
  const topics = payload.topics;
  if (topics.length < 3) {
    runtime4.log("Log payload does not contain enough topics");
    throw new Error(`log payload does not contain enough topics ${topics.length}`);
  }
  const emitter = bytesToHex(topics[1].slice(12));
  runtime4.log(`Emitter ${emitter}`);
  const message6 = getLastMessage(runtime4, runtime4.config.evms[0], emitter);
  runtime4.log(`Message retrieved from the contract ${message6}`);
  return message6;
};
var onHTTPTrigger = (runtime4, payload) => {
  runtime4.log("Raw HTTP trigger received");
  if (!payload.input || payload.input.length === 0) {
    runtime4.log("HTTP trigger payload is empty; defaulting execution time to now");
    return doPOR(runtime4);
  }
  runtime4.log(`Payload bytes payloadBytes ${payload.input.toString()}`);
  try {
    runtime4.log(`Parsed HTTP trigger received payload ${payload.input.toString()}`);
    return doPOR(runtime4);
  } catch (error4) {
    runtime4.log("Failed to parse HTTP trigger payload");
...
var initWorkflow = (config2) => {
  ...
  return [
    ...
    cre.handler(evmClient.logTrigger({
      addresses: [config2.evms[0].messageEmitterAddress]
    }), onLogTrigger),
