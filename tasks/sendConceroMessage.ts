...
task("send-concero-message", "Send a test Concero message through the client")
    ...
    .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
        ...
        if (!dstNetwork) {
            throw new Error(`Destination network '${taskArgs.dstnetwork}' not found in conceroNetworks`);
        }
        ...
        if (!dstChainClient) {
            throw new Error(`Destination chain client address not found for network: ${dstNetwork.name}`);
        }
        ...
        if (concurrency === 1) {
            ...
            const messagePromises = Array(concurrency).fill(null).map((_, index) => {
                return sendSingleMessage({
                    ...messageParams,
                    logPrefix: `send-concero-message-${index + 1}`
                }).catch(error => {
                    log(`Message ${index + 1} failed: ${error.message}`, "send-concero-message", "error");
                    return null;
                });
            });
            ...
            log(`Successfully sent ${successfulResults.length}/${concurrency} concurrent messages`, "send-concero-message");
