...
import { getEnvVar, getHashSum, log, updateEnvVariable } from "../../../utils";
...
const deployVerifierHarness: DeploymentFunction = async function (
	hre: HardhatRuntimeEnvironment,
	overrideArgs?: Partial<DeployArgs>,
): Promise<Deployment> {
	...
	const chain = conceroNetworks[name as ConceroNetwork];
	...
	const deployment = await deploy("ConceroVerifierHarness", {
		from: deployer,
		args: [
			args.chainSelector,
			args.usdc,
